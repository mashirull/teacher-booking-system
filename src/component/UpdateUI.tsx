...
const UpdateUI = ({ appointment }) => {
    ...
    return (
        <TableCell sx={appointment.status === "pending" ? { color: "#9b8401" } : appointment.status === "approved" ? { color: "green" } : { color: "red" }}>
            {appointment.status}
            ...
            <Button
                ...
                disabled={isDisable}
                onClick={() => dispatch(updateAppointmentStatus({ appointmentId: appointment._id, status }))}
            >
                update
            </Button>
        </TableCell>
